...
void Electron2D::interactRK_ActorE(const RFField& E, const MagneticField& B, double time_interval, shared_ptr<mutex>& E_mutex, shared_ptr<mutex>& B_mutex){
    vector3d run_kut_E = interactE_RK(E, time_interval, E_mutex);
    vector3d run_kut_B = interactB_RK(B, time_interval, B_mutex);
...
vector3d Electron2D::interactE_RK(const RFField& E, double time_interval, shared_ptr<mutex>& E_mutex){
    Electron2D e_dummy;
    e_dummy.Et = Et;
    e_dummy.pos = pos;
    e_dummy.vel = vel;
...
vector3d Electron2D::interactB_RK(const MagneticField& B, double time_interval, shared_ptr<mutex>& B_mutex){
    if (B.isInside(pos) == -1){
        return vector3d(0,0,0);
    }
    Electron2D e_dummy;
